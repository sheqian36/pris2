...
def rgb2lab(rgb):
    ...
    l_rs = (lab[:,[0],:,:]-50.)/100. *2.# [-1,1] default: l_cent = 50; l_norm = 100 
    ab_rs = lab[:,1:,:,:]/110. # opt.ab_norm = 110. ab:[-1,1]
...
def get_colorization_data(data_raw, ab_thresh=5., p=.125, num_points=None):
    ...
    data_lab = rgb2lab(data_raw)
    data['real_rgb'] = data_raw
    data['A'] = data_lab[:,[0,],:,:]
    data['B'] = data_lab[:,1:,:,:]
...
def tensor2im(input_image, imtype=np.uint8):
    ...
    for i in range(image_tensor.shape[0]):
        ...
        imgs.append(image_numpy.astype(imtype))
    return imgs
